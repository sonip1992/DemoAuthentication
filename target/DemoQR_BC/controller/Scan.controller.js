...
sap.ui.define([
	...
	return Controller.extend("com.demo.qr_bc.DemoQR_BC.controller.Scan", {
		...
		onVerify: function (oEvent) {
			...
			if(!!this._scannedBarcode) {
				_valueToVerify = this._scannedBarcode;
				_type = "bc";
				_typeExt = "Barcode"
			}
			
			if(!!this._scannedQR) {
				_valueToVerify = this._scannedQR;
				_type = "qr";
				_typeExt = "QR";
			}
			
			var response = jQuery.sap.syncGetJSON("Delegate", {"value": _valueToVerify, "type": _type}).data;
			var isValueValid = response.result;
			
			if(isValueValid) {
				MessageBox.success("Entered " + _typeExt + ": " + _valueToVerify + " is valid.");
			} else {
				MessageBox.error("Entered " + _typeExt + ": " + _valueToVerify + " is not valid.");
			}
		},
