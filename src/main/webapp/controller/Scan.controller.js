...
sap.ui.define([
	...
	], function (Controller, MessageBox, quaggajs, grid, version, detector, formatinf, errorlevel, bitmat, datablock, bmparser, datamask, rsdecoder,
	...
	return Controller.extend("com.demo.qr_bc.DemoQR_BC.controller.Scan", {
		onScanForBC: function (oEvent) {
			if(this.getView().byId("scannedQRValue").getValue() !== "") {
				this.getView().byId("scannedQRValue").setValue("");
			}
		...
		_initQuagga: function (oTarget) {
			...
			if (!this._bQuaggaEventHandlersAttached) {
				...
				Quagga.onDetected(function (result) {
					...
					this._scannedBarcode = result.codeResult.code;
					this.getView().byId("scannedValue").setValue(this._scannedBarcode);
		...
		onScanForQR: function (oEvent) {
			if(this.getView().byId("scannedValue").getValue() !== "") {
		...
		onVerify: function (oEvent) {
			var _valueToVerify, _type, _typeExt;
			if(this.getView().byId("scannedValue").getValue() !== "") {
				_valueToVerify = this.getView().byId("scannedValue").getValue();
				_type = "bc";
				_typeExt = "Barcode"
			}

			if(this.getView().byId("scannedQRValue").getValue() !== "") {
				_valueToVerify = this.getView().byId("scannedQRValue").getValue();
				_type = "qr";
				_typeExt = "QR";
			}

			var jsonData = {
					"value": _valueToVerify, 
					"type": _type
			...
			var response = jQuery.sap.syncGetJSON("./Delegate", data).data;
			var isValueValid = response.result;

			if(isValueValid) {
				MessageBox.success("Entered " + _typeExt + ": " + _valueToVerify + " is valid with Manufacturer.");
			} else {
				MessageBox.error("Entered " + _typeExt + ": " + _valueToVerify + " is not valid with Manufacturer.");
			}
		},
